...
if(isset($_POST['submit']))
{
    ...
    $mail=$_POST['mail'];
    $sql1="SELECT email,password,username FROM users WHERE email='{$mail}'  LIMIT 1";
    $q= $mysqli->query($sql1);
    ...
    if($q->num_rows == 1)
    {
        ...
        if($password==$resetcode)
        {
            ...
            if ($mysqli->query($sql2)) {
                ...
                echo "<script>window.location.assign('resetdone.html')</script>";
                exit();
        ...
        {
            $statusErr='The reset code you entered is not valid.';
        }
    }
    else
    {
        $statusErr='You entered E-mail address is not present.';
    }
}
...
<div class="container col s6">
    ...
    <div class="row" style="background-color: #757575; margin-bottom: 8em;">
        <div class="col l8 offset-l2 m10 offset-m1 s12 hoverable" style="background-color:white;">
            <br /><br /><br />
            <div class="col s12" >
                <h5  style="text-align: center; font-family: Roboto-Light,Raleway;">Reset Password</h5>
                <form action="<?php echo $_SERVER['PHP_SELF'] ?>" method='post'>
                    <table cellspacing='5' align='center'>
                        <tr>
                            <td>Email Address:</td>
                            <td><input title="E-Mail" type='email' name='mail' required/></td>
                        </tr>
                        <tr>
                            <td>Reset Code:</td>
                            <td><input title="Reset Code" type='text' name='resetcode' required/></td>
                        </tr>
                        <tr>
                            <td>New Password:</td>
                            <td><input id="form_pass" title="New Password" type='password' name='newpwd' pattern=".{8,}" required/></td>
                        </tr>
                        <tr>
                            <td>Confirm Password:</td>
                            <td><input id="form_cnfrmpass" title="Re-Enter Your New Password" type='password' name='confirm_password' pattern=".{8,}" required/></td>
                        </tr>

                        <tr>
                            <td></td>
                            <td>
                                <button class="btn waves-effect waves-light indigo accent-3" type="submit" name="submit">Submit
                                    <i class="material-icons right">send</i>
                                </button>
                            </td>
                        </tr>
                    </table>
                    <p style="color: green;"><?php echo $statusOk; ?></p>
